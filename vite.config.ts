  ...
  return {
    ...
    load(id: string) {
      ...
      return [
        ...
        "  window.addEventListener('error', (e) => { send({ type: 'runtime-error', message: e.message, stack: e.error?.stack, filename: e.filename, lineno: e.lineno, colno: e.colno }); });",
        "  window.addEventListener('unhandledrejection', (e) => { const err = e.reason; send({ type: 'unhandled-rejection', message: err?.message || String(err), stack: err?.stack }); });",
        "}",
      ].join("\n");
    },
    configureServer(server: import("vite").ViteDevServer) {
      ...
      console.error = (...args: unknown[]) => {
        ...
        try {
          ...
          if (error instanceof Error) {
            server.ws.send({ type: "custom", event: "client-runtime-error", data: { source: "ssr", type: "ssr-render-error", name: error.name, message: error.message, stack: error.stack } });
          }
      ...
      server.ws.on("client-runtime-error", (data: Record<string, string>) => {
        ...
        let loc = "";
        if (filename) { loc = ` at ${filename}`; if (lineno != null) loc += `:${lineno}`; if (colno != null) loc += `:${colno}`; }
        server.config.logger.error(`\n[client] ${label}: ${message}${loc}`);
        if (stack) server.config.logger.error(stack);
        server.ws.send({ type: "custom", event: "client-runtime-error", data });
      });
    },
    transform(code: string, id: string) {
      const normalizedId = id.replace(/\\/g, "/");
      if (normalizedId.includes("routes/__root")) return `import "${VIRTUAL_ID}";\n${code}`;
    },
  };
}
...
function devServerFnErrorLogger() {
  ...
  return {
    ...
    enforce: "pre" as const,
    configureServer(server: import("vite").ViteDevServer) {
      (globalThis as Record<string, unknown>)[HMR_SEND_KEY] = (data: unknown) => { server.ws.send({ type: "custom", event: "server-fn-error", data }); };
    },
    transform(code: string, id: string) {
      const normalizedId = id.replace(/\\/g, "/");
      ...
      return code.replace(needle, `${needle}\n\n      if (res?.error) {\n        const err = res.error\n        const payload = { source: 'tanstack', type: 'server-fn-error', method: request.method, url: request.url, name: err?.name ?? 'Error', message: err?.message ?? String(err), stack: typeof err?.stack === 'string' ? err.stack : undefined }\n        globalThis.${HMR_SEND_KEY}?.(payload)\n      }`);
    },
  };
}
